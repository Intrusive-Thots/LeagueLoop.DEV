...
def main() -> int:
    ...
    parser.add_argument(
        "--allow-multiple",
        action="store_true",
        help="Run a second copy alongside one that is already running. "
             "They share the cache, config and account store, so only do "
             "this deliberately.",
    )
    ...
    if args.screenshot:
        ...
        Logger.info("Startup", f"Saved screenshot to {args.screenshot}")
