...
class AccountsTool(ctk.CTkFrame):
    ...
    def _build_body(self):
        ...
        # ── Client Launcher Bar ──
        launch_bar = ctk.CTkFrame(self.body, fg_color="transparent")
        launch_bar.pack(fill="x", padx=4, pady=(2, 6))

        btn_rc = ctk.CTkButton(
            launch_bar,
            text="🚀 Riot Client",
            font=get_font("caption", "bold"),
            height=26,
            ...
            command=self._launch_riot_client_only,
            cursor="hand2"
        )
        btn_rc.pack(side="left", fill="x", expand=True, padx=(0, 2))
        CTkTooltip(btn_rc, "Launch Riot Games Client launcher")

        btn_lol = ctk.CTkButton(
            launch_bar,
            text="⚔️ League Client",
            font=get_font("caption", "bold"),
            height=26,
            fg_color=get_color("colors.accent.primary"),
            hover_color=get_color("colors.state.hover"),
            text_color="#ffffff",
            command=self._launch_league_client,
            cursor="hand2"
        )
        btn_lol.pack(side="left", fill="x", expand=True, padx=(2, 0))
        CTkTooltip(btn_lol, "Launch Riot Client and start League of Legends")
