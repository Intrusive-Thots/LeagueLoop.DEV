...
class SettingsPanel(ctk.CTkScrollableFrame):
    ...
    def _build_social(self):
        card_social = make_card(self, title="QUEQQ", padx=0, pady=(0, SECTION_GAP), collapsible=True, start_collapsed=True)
        ...
        self.vip_var.trace_add("write", _save_vip)
    ...
    def _build_hotkeys(self):
        ...
        for label_text, config_key, default_val in hotkeys:
            ...
            row.pack(fill="x", pady=(0, INNER_GAP))
            ctk.CTkLabel(row, text=label_text, font=get_font("body"), text_color=get_color("colors.text.primary")).pack(side="left")
            recorder = HotkeyRecorder(row, current_hotkey=self.config.get(config_key, default_val), on_change=lambda val, k=config_key: self._save_hotkey(k, val))
    ...
    def _build_profile(self):
        card_profile = make_card(self, title="PROFILE", padx=0, pady=(0, SECTION_GAP), collapsible=True, start_collapsed=True)

        ctk.CTkLabel(card_profile, text="Custom Status", font=get_font("caption"), text_color=get_color("colors.text.muted")).pack(anchor="w", pady=(0, 2))
        ...
        self.preset_frame = ctk.CTkFrame(card_profile, fg_color="transparent")
        self.preset_frame.pack(fill="x", pady=(0, 0))

        presets = [
            ("🚀", "Grinding Ranked"),
            ("🎮", "Queqq ⚙️ https://github.com/Intrusive-Thots/LeagueLoop-Installer"),
            ("🌮", "Eating / Brb"),
            ("💤", "AFK"),
        ]
    ...
    def _build_about(self):
        ...
        ctk.CTkLabel(card_about, text="Queqq", font=get_font("title", "bold"), text_color=get_color("colors.text.primary")).pack(anchor="w")
