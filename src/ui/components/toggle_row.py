...
class ToggleRow(ctk.CTkFrame):
    ...
    def __init__(self, master, label_text, variable, command, tooltip_text="",
        ...
        # Hover bindings for edit button
        if self._on_edit:
            self.bind("<Enter>", self._show_edit)
            self.bind("<Leave>", self._hide_edit)
            for w in [self.icon_label, self.text_label]:
                w.bind("<Enter>", self._show_edit)
                w.bind("<Leave>", self._hide_edit)
    ...
    def _show_edit(self, event=None):
        ...
        if self._on_edit and self.winfo_exists():
            self.btn_edit.pack(side="right", padx=(0, 4))
    
    def _hide_edit(self, event=None):
        """Hide edit button when not hovering."""
        if self.winfo_exists():
            # Check if mouse is still within this row
            try:
