...
class GridEmptyStateTests(unittest.TestCase):
    """The grid must explain a failed download and offer a way out of it."""
    ...
    def _grid(self, champ_data, error="", retry=None):
        from ui.qt.widgets.champion_grid import QtChampionGrid

        class Assets:
            def __init__(self):
                self.champ_data = champ_data
                self.champion_data_error = error
                if retry is not None:
                    self.retry_champion_data = retry

        grid = QtChampionGrid(asset_manager=Assets())
        grid.load_champions()
        return grid

    def test_a_failure_reason_is_shown(self):
        grid = self._grid({}, error="ConnectionError: no route to host")
        self.assertIn("no route to host", grid.empty_label.text())

    def test_no_reason_yet_reads_as_still_downloading(self):
        grid = self._grid({})
        self.assertIn("not finished downloading", grid.empty_label.text())
        self.assertFalse(grid.btn_retry.isVisibleTo(grid))

    def test_retry_is_offered_only_when_it_can_do_something(self):
        grid = self._grid({}, error="boom", retry=lambda: True)
        self.assertTrue(grid.btn_retry.isVisibleTo(grid))

        no_api = self._grid({}, error="boom")
        self.assertFalse(no_api.btn_retry.isVisibleTo(no_api))

    def test_retry_reloads_the_grid_when_it_succeeds(self):
        from PySide6.QtCore import QThreadPool

        state = {"data": {}}

        def retry():
            state["data"]["Ahri"] = {"key": "103", "name": "Ahri"}
            return True

        grid = self._grid(state["data"], error="boom", retry=retry)
        self.assertEqual(grid.tiles, {})

        grid.btn_retry.click()
        QThreadPool.globalInstance().waitForDone(2000)
        self.app.processEvents()

        self.assertIn(103, grid.tiles)

    def test_retry_is_not_reentrant(self):
        calls = []
        grid = self._grid({}, error="boom", retry=lambda: calls.append(1))
        grid._retrying = True
        grid._on_retry()
        self.assertEqual(calls, [])

    def test_a_populated_grid_never_shows_the_retry(self):
        grid = self._grid({"Ahri": {"key": "103", "name": "Ahri"}})
        self.assertFalse(grid.btn_retry.isVisibleTo(grid))
        self.assertFalse(grid.empty_label.isVisibleTo(grid))
